...
class ParserState {
  ...
  constructor(
    ...
    this.previous_lexer = previous_lexer
    this.lexer = lexer
  ...
  advance(): Token {
    assert(
      this.current_token.kind !== t.EndOfFile,
      "Tried to advance past the EOF",
    )
    this.previous_lexer = this.lexer.clone()
    this.last_token = this.current_token
    this.current_token = this.lexer.next()
    if (this.at(t.Error)) {
      emit_error(this, this.current_token.text)
    }
    assert(this.current_token.span.start >= this.last_token.span.start)
  ...
  parse_comma_separated_list<T>(
    ...
    while (
      this.current_token.kind !== t.EndOfFile &&
      this.current_token.kind !== end_token
    ) {
      if (!first) {
        if (this.at(t.Comma)) {
          this.advance()
        } else {
          emit_error(this, `Expected a comma or ${end_token}`)
          return ERR
        }
      } else {
        first = false
      ...
      if (this.at(t.Comma) && this.next_is(end_token)) {
        this.advance()
        break
      }
    }
  ...
  next_is(token_kind: TokenKind): boolean {
    return this.lexer.clone().next().kind === token_kind
  }

  parse_ident(): Ident | Err {
    if (this.at(t.Ident)) {
      const tok = this.advance()
      return { span: tok.span, text: tok.text }
    } else {
      emit_error(this, "Expected an identifier")
      return ERR
    }
  }

  expect(token_kind: TokenKind): Token | Err {
    if (this.current_token.kind === token_kind) {
      return this.advance()
    } else {
      emit_error(this, `Expected ${token_kind}, got ${this.current_token.kind}`)
      return ERR
    }
  }
}
...
function parser_impl(path: string, source: string, flags: number): Parser {
  ...
  function parse_update_expr(): Expr | Err {
    switch (self.current_token.kind) {
      ...
      default: {
        ...
        if (self.at(t.PlusPlus) && !current_is_on_new_line()) {
          const end = self.advance()
          return Expr.PostIncrement(Span.between(lhs.span, end.span), lhs)
        } else if (self.at(t.MinusMinus) && !current_is_on_new_line()) {
          const end = self.advance()
    ...
    while (true) {
      switch (current_kind()) {
        ...
        case t.LSquare: {
          ...
          if (prop === ERR) return prop
          const end = self.expect(t.RSquare)
  ...
  function parse_arguments(): Err | ArgsWithSpan {
    ...
    const args_or_null = self.parse_comma_separated_list(t.RParen, () => {
      if (self.at(t.DotDotDot)) {
        ...
        if (
          ...
          !(self.current_token.kind === t.Comma && self.next_is(t.RParen))
        ) {
          emit_error(self, "Spread operator must be the last argument")
          return ERR
        }
        return null
      }
    ...
    const last = self.expect(t.RParen)
  ...
  function parse_primary_expr(): Expr | Err {
    switch (self.current_token.kind) {
      ...
      case t.True:
        return Expr.Boolean(self.advance().span, true)
      case t.False:
        return Expr.Boolean(self.advance().span, false)
      case t.Null:
        return Expr.Null(self.advance().span)
      case t.Undefined:
        return Expr.Undefined(self.advance().span)
      case t.String: {
      ...
      case t.LBrace: {
        const start = self.advance()
        const entries = self.parse_comma_separated_list(
          t.RBrace,
          parse_object_literal_entry,
        )
        if (entries === ERR) return ERR
        const end = self.expect(t.RBrace)
      ...
      case t.Slash: {
        re_lex_regex()
        if (!self.at(t.Regex) && !self.at(t.Error)) {
        ...
        const tok = self.expect(t.Regex)
        if (tok === ERR) return ERR
        return Expr.Regex(tok.span, tok.text)
      }
      case t.Super:
        return Expr.Super(self.advance().span)
      case t.Class: {
      ...
      default:
        emit_error(self, "Expected an expression")
        return ERR
    }
  }
  ...
  function parse_template_literal(): Expr | Err {
    ...
    while (true) {
      if (self.at(t.TemplateLiteralFragment)) {
        const tok = self.advance()
        ...
        emit_error(self, "Unexpected end of template literal")
        return ERR
      }
    }
  ...
  function parse_class(): Class | Err {
    const first = self.expect(t.Class)
    ...
    const body_start = self.expect(t.LBrace)
    if (body_start === ERR) return ERR

    const members: ClassMember[] = []
    while (!self.at(t.RBrace) && !self.at(t.EndOfFile)) {
      const member = parse_class_member()
      if (member === ERR) return ERR
      members.push(member)
    }

    const body_end = self.expect(t.RBrace)
  ...
  function parse_method_def(): MethodDef | Err {
    ...
    const is_async = self.at(t.Async)
    if (is_async) {
      self.advance()
    }

    const is_generator = self.at(t.Star)
    if (is_generator) {
      self.advance()
    }
  ...
  function parse_object_literal_entry(): ObjectLiteralEntry | Err {
    if (self.at(t.DotDotDot)) {
      ...
      self.at(t.Ident) &&
      (self.next_is(t.Comma) || self.next_is(t.RBrace))
    ) {
      const ident = self.parse_ident()
      if (ident === ERR) return ERR
    ...
    } else if (self.at(t.Async) && self.next_is(t.Star)) {
      return parse_object_literal_entry_method()
    } else if (self.at(t.Async) && self.next_is(t.Ident)) {
      return parse_object_literal_entry_method()
    } else if (self.at(t.Async) && self.next_is(t.LSquare)) {
      return parse_object_literal_entry_method()
    } else if (self.at(t.Async) && next_token_is_keyword()) {
      return parse_object_literal_entry_method()
    } else if (self.at(t.Star)) {
      return parse_object_literal_entry_method()
    } else if (self.at(t.Ident) && self.next_is(t.LParen)) {
      return parse_object_literal_entry_method()
    } else {
      ...
      switch (self.current_token.kind) {
        ...
        default: {
          if (self.expect(t.Colon) === ERR) return ERR
          const expr = parse_assignment_expr()
          if (expr === ERR) return ERR
  ...
  function parse_object_literal_entry_method(): ObjectLiteralEntry | Err {
    ...
    if (accessor_type === AccessorType.Get && params.params.length > 0) {
      emit_error(self, `Getter method should not have any parameters`)
  ...
  function parse_object_key(): ObjectKey | Err {
    switch (self.current_token.kind) {
      ...
      case t.LSquare: {
        ...
        const expr = parse_expr()
        if (expr === ERR) return ERR
        if (self.expect(t.RSquare) === ERR) return ERR
  ...
  function parse_params_with_parens(): ParamList | Err {
    const start = self.expect(t.LParen)
    if (start === ERR) return ERR
    const params: Param[] = []

    while (!self.at(t.RParen) && !self.at(t.EndOfFile)) {
      if (params.length > 0) {
        if (self.expect(t.Comma) === ERR) return ERR
      }
      ...
      if (self.at(t.Comma) && self.next_is(t.RParen)) {
        self.advance()
        break
      }
    }

    const stop = self.expect(t.RParen)
  ...
  function parse_optional_type_annotation(): TypeAnnotation | null | Err {
    if (flags & PARSER_FLAGS.ALLOW_TYPE_ANNOTATIONS && self.at(t.Colon)) {
      self.advance()
  ...
  function parse_array_type_annotation_or_below(): TypeAnnotation | Err {
    ...
    loop: while (true) {
      switch (self.current_token.kind) {
        ...
        case t.LessThan: {
          self.advance()
          const args = self.parse_comma_separated_list(
            t.GreaterThan,
            parse_type_annotation,
          )
          if (args === ERR) return ERR
          const end = self.expect(t.GreaterThan)
          if (end === ERR) return ERR
    ...
    switch (self.current_token.kind) {
      case t.Ident: {
        if (self.current_token.text === "readonly") {
          ...
          if (annot.kind !== "Array") {
            emit_error(self, "The readonly can only be applied to array types")
            return annot
          }
      ...
      default:
        emit_error(self, "Expected a type annotation")
        return ERR
    }
  }
  function parse_generic_func_type_annotation(): TypeAnnotation | Err {
    ...
    if (type_params === ERR) return ERR
    if (self.expect(t.LParen) === ERR) return ERR
    const params = self.parse_comma_separated_list(
      t.RParen,
      parse_func_type_param,
    )
    if (params === ERR) return ERR
    if (self.expect(t.RParen) === ERR) return ERR
    if (self.expect(t.FatArrow) === ERR) return ERR
  ...
  function parse_parenthesized_or_func_type_annotation(): TypeAnnotation | Err {
    ...
    if (self.at(t.RParen) || (self.at(t.Ident) && self.next_is(t.Colon))) {
      const params = self.parse_comma_separated_list(
        t.RParen,
        parse_func_type_param,
      )
  ...
  function parse_comma_semi_or_newline_separated_list<T>(
    ...
    while (
      ...
      if (!first) {
        if (self.at(t.Comma) || self.at(t.Semi)) {
          self.advance()
        } else if (
          self.last_token &&
          ...
          emit_error(self, `Expected a comma, ; a newline or ${end_token}`)
          return ERR
        }
      } else {
        first = false
  ...
  function parse_paren_expr(): Expr | Err {
    ...
    const expr = parse_expr()
    if (expr === ERR) return ERR
    const end = self.expect(t.RParen)
    if (end === ERR) return ERR
  ...
  function parse_assignment_expr(): Expr | Err {
    if (self.at(t.Yield)) {
    ...
    } else if (self.at(t.LParen) && can_start_arrow_fn()) {
      return parse_arrow_fn()
    } else {
      ...
      if (assignOp !== null) {
        ...
        if (lhsPattern === null) {
          emit_error(self, "Invalid left-hand side in assignment")
          return ERR
        }
    ...
    switch (self.current_token.kind) {
      ...
      case t.DotDotDot: {
        if ((flags & PatternFlags.ALLOW_SPREAD) !== 0) {
          ...
          emit_error(self, "Unexpected rest pattern")
          return ERR
        }
      }
      case t.LSquare: {
      ...
      case t.LBrace: {
        ...
        while (true) {
          if (self.at(t.RBrace) || self.at(t.EndOfFile)) {
            break
          } else if (self.at(t.DotDotDot)) {
            self.advance()
        ...
        const stop = self.expect(t.RBrace)
        if (stop === ERR) return ERR
        head = Pattern.Object(
      ...
      default:
        emit_error(self, `Expected a pattern, got ${self.current_token.kind}`)
        return ERR
    }

    if (self.at(t.Eq) && (flags & PatternFlags.ALLOW_ASSIGNMENT) !== 0) {
      self.advance()
      const init = parse_assignment_expr()
      if (init === ERR) return ERR
  ...
  function parse_block(): Block | Err {
    const start = self.expect(t.LBrace)
    if (start === ERR) return ERR
  ...
  function parse_array_literal(): Expr | Err {
    ...
    while (true) {
      if (
        (self.at(t.Comma) && self.next_is(t.RSquare) && elements.length > 0) ||
        self.at(t.RSquare) ||
        self.at(t.EndOfFile)
      ) {
        if (self.at(t.Comma)) self.advance()
        break
      } else if (self.at(t.DotDotDot)) {
        const start = self.advance()
        const expr = parse_assignment_expr()
  ...
  function parse_stmt(export_token: Token | null = null): Stmt | Err {
    if (
      self.at(t.Export) &&
      (self.next_is(t.Function) ||
        self.next_is(t.Class) ||
        self.next_is(t.Var) ||
        self.next_is(t.Let) ||
        self.next_is(t.Const) ||
    ...
    switch (self.current_token.kind) {
      ...
      case t.Ident: {
        if (self.next_is(t.Colon)) {
          const label = self.parse_ident()
          assert(label !== ERR) // because of the lookahead above
          assert(self.advance().kind === t.Colon) // because of the lookahead above
          const stmt = parse_stmt()
      ...
      case t.Break: {
        const span = self.advance().span
        if (self.at(t.Ident) && !current_is_on_new_line()) {
      ...
      case t.Async: {
        if (self.next_is(t.Function)) {
          ...
          emit_error(self, `Expected 'function'`)
          return ERR
        }
      }
      case t.Import:
  ...
  function parse_import_stmt(): Stmt | Err {
    ...
    if (default_import) {
      if (self.at(t.Comma)) {
        self.advance()
      } else if (self.at(t.From)) {
        self.advance()
        const from_clause = self.expect(t.String)
        ...
        emit_error(self, "Expected a comma or 'from'")
        return ERR
      }
    }
    if (self.expect(t.LBrace) === ERR) return ERR
    const import_specifiers = self.parse_comma_separated_list(
      t.RBrace,
      parse_import_specifier,
    )
    if (import_specifiers === ERR) return ERR
    if (self.expect(t.RBrace) === ERR) return ERR
    if (self.expect(t.From) === ERR) return ERR
    const from_clause = self.expect(t.String)
    if (from_clause === ERR) return ERR
    if (expect_semi() === ERR) return ERR
    return Stmt.Import(
  ...
  function parse_type_decl(): Stmt | Err {
    ...
    const ident = self.parse_ident()
    if (ident === ERR) return ERR
    if (self.expect(t.Eq) === ERR) return ERR
    if (self.at(t.LBrace)) {
      self.advance()
      ...
      const last = self.expect(t.RBrace)
      if (last === ERR) return ERR
  ...
  function parse_import_specifier(): ImportSpecifier | Err {
    const start = self.current_token.span
    let type_only = false
    if (self.at(t.Ident) && self.current_token.text === "type") {
      self.advance()
      type_only = true
    }
    if (self.at(t.String)) {
      const tok = self.advance()
      if (self.expect(t.As) === ERR) return ERR
      const alias = self.parse_ident()
  ...
  function parse_for_stmt(): Stmt | Err {
    ...
    let init: ForInit
    if (self.at(t.Let) || self.at(t.Const) || self.at(t.Var)) {
      const decl = parse_var_decl()
      if (decl === ERR) return ERR
      init = ForInit.VarDecl(decl)
    } else if (self.at(t.Semi)) {
      self.advance()
      ...
      // No ASI allowed here
      if (self.expect(t.Semi) === ERR) return ERR
  ...
  function parse_for_in_of_stmt(): Stmt | Err {
    ...
    switch (self.current_token.kind) {
      ...
      default:
        emit_error(
          self,
          `Expected 'in' or 'of', got ${self.current_token.kind}`,
        )
        return ERR
    }
  ...
  function parse_switch_stmt(): Stmt | Err {
    const start_token = self.expect(t.Switch)
  ...
  function parse_switch_case(): SwitchCase | Err {
    if (self.at(t.Case)) {
      const start = self.expect(t.Case)
      if (start === ERR) return ERR
      const expr = parse_expr()
      if (expr === ERR) return ERR
      const colon = self.expect(t.Colon)
      ...
      const start = self.expect(t.Default)
      if (start === ERR) return ERR
      const colon = self.expect(t.Colon)
      if (colon === ERR) return ERR
  ...
  function parse_while_stmt(): Stmt | Err {
    const start = self.advance().span // Consume the 'while' keyword
  ...
  function parse_do_while_stmt(): Stmt | Err {
    const start = self.expect(t.Do)
    if (start === ERR) return ERR

    const body = parse_stmt()
    if (body === ERR) return ERR

    if (self.expect(t.While) === ERR) return ERR
    if (self.expect(t.LParen) === ERR) return ERR
  ...
  function parse_return_stmt(): Stmt | Err {
    const start = self.expect(t.Return)
    if (start === ERR) return ERR
  ...
  function parse_function(): Func | Err {
    ...
    const name = self.at(t.Ident) ? self.parse_ident() : null
    if (name === ERR) return ERR
    const type_params = parse_optional_type_params()
    if (type_params === ERR) return ERR
  ...
  function parse_type_params(): readonly TypeParam[] | Err {
    ...
    if (params === ERR) return ERR
    if (self.expect(t.GreaterThan) === ERR) return ERR
  ...
  function can_start_arrow_fn(): boolean {
    ...
    const restore = fork()
    if (!self.expect(t.LParen)) return restore(false)
    const params = self.parse_comma_separated_list(t.RParen, parse_param)
    if (params === ERR) return restore(false)
    const rparen = self.expect(t.RParen)
    if (rparen === ERR) return restore(false)
    const last = self.advance()
    if (
  ...
  function parse_if_stmt(): Stmt | Err {
    ...
    if (!self.at(t.LParen)) {
      emit_error(self, "Expected '(' after 'if'")
    } else {
      parenthesized = true
      self.advance()
    }
    ...
    const else_branch = self.at(t.Else) ? (self.advance(), parse_stmt()) : null
  ...
  function expect_semi(): Err | void {
    ...
    emit_error(self, "Missing semicolon")
    return ERR
  }
  ...
  function recover_till_next_stmt(): void {
    ...
    do {
      ...
      if (
        ...
        (self.at(t.Ident) && self.next_is(t.LParen)) ||
        (self.at(t.Ident) && self.current_token.text === "type")
      ) {
        break
      }
    } while (true)
  ...
  function define_binop_parser(
    ...
    return function () {
      ...
      while (op_kinds.has(self.current_token.kind)) {
        const op = parse_bin_op(self.advance().kind)
        const rhs = next_fn()
        if (rhs === ERR) return ERR
...
function emit_error(self: ParserState, message: string): void {
  self.errors.push({
    message,
